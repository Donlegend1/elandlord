...
export default function Edit({ product }) {
  ...
  const { data, setData, put, processing, errors } = useForm({
    ...
    image: product.image || '',
    images: specs.images || (product.image ? [product.image] : []),
    price: specs.price || '',
    location: specs.location || '',
    type: specs.type || 'Villa',
    bedrooms: specs.bedrooms || '',
    bathrooms: specs.bathrooms || '',
    area: specs.area || '',
    status: specs.status || 'For Sale',
  ...
  const submit = e => {
    e.preventDefault();
    put(`/admin/properties/${product.id}`, {
      onSuccess: () => alert('Property updated successfully!')
    });
  };

  return (
    <Authenticated header="Edit Property Listing">
      <div className="max-w-4xl mx-auto space-y-6">
        
        {/* Header */}
        <div>
          <h2 className="font-serif text-3xl font-bold text-slate-900">Edit Property: {product.name}</h2>
          <p className="text-slate-500 text-sm mt-1">Make changes to the property details below.</p>
        </div>
